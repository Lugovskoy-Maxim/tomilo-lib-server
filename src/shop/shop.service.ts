...
export class ShopService {
  ...
  /**
   * Одно активное предложение на пользователя. После еженедельного приёма (понедельник)
   * победители остаются в БД со статусом accepted — их нельзя учитывать в лимите, иначе
   * они не смогут предложить декорацию на новую неделю, пока не истечёт 7 дней с createdAt.
   * Непобедители (pending вне топа) удаляются — у них записи нет.
   */
  private async checkUserCanSuggest(userId: string): Promise<void> {
    ...
    if (existing) {
      throw new BadRequestException(
        'У вас уже есть предложение на рассмотрении. Дождитесь итогов голосования в понедельник или отредактируйте текущее предложение в течение часа после отправки.',
      );
    }
  }
